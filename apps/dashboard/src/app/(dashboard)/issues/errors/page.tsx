...
export default async function ErrorsPage() {
    const issues = await getAllOrgIssues({ severity: "ERROR" });

    return (
        <div className="space-y-8 pb-12">
            <div className="halo-page-header">
                <h1 className="halo-page-title">Errors</h1>
                <p className="halo-page-description">
                    Issues tagged with ERROR severity.
                </p>
            </div>
            ...
            {issues.length === 0 ? (
                <div className="halo-empty-state">
                    ...
                    <h3 className="halo-empty-state-title">No errors found</h3>
                    <p className="halo-empty-state-description">No ERROR severity issues recorded.</p>
                </div>
            ) : (
