...
export default async function IgnoredPage() {
    const issues = await getAllOrgIssues({ status: "IGNORED" });

    return (
        <div className="space-y-8 pb-12">
            <div className="halo-page-header">
                <h1 className="halo-page-title">Ignored Issues</h1>
                <p className="halo-page-description">
                    Issues muted or marked as ignored.
                </p>
            </div>

            {issues.length === 0 ? (
                <div className="halo-empty-state">
                    <AlertCircle className="halo-empty-state-icon" />
                    <h3 className="halo-empty-state-title">No ignored issues</h3>
                    <p className="halo-empty-state-description">No ignored issues found.</p>
                </div>
            ) : (
