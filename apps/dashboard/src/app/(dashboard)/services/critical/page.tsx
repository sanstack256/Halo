...
export default async function CriticalServicesPage() {
    ...
    const filtered = services.filter((s) => s.health === "critical");

    return (
        <div className="space-y-8 pb-12">
            <div className="halo-page-header">
                <h1 className="halo-page-title">Critical Services</h1>
                <p className="halo-page-description">Services with severe failure rates (&gt;= 20% error rate).</p>
            </div>

            {filtered.length === 0 ? (
                <div className="halo-empty-state">
                    <Server className="halo-empty-state-icon" />
                    <h3 className="halo-empty-state-title">No critical services</h3>
                    <p className="halo-empty-state-description">No services currently in critical status.</p>
                </div>
            ) : (
