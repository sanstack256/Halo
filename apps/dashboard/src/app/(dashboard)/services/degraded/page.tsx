...
export default async function DegradedServicesPage() {
    const services = await getServices();
    const filtered = services.filter((s) => s.health === "degraded");

    return (
        <div className="space-y-8 pb-12">
            <div className="halo-page-header">
                <h1 className="halo-page-title">Degraded Services</h1>
                <p className="halo-page-description">Services experiencing elevated error rates (5% - 20%).</p>
            </div>

            {filtered.length === 0 ? (
                <div className="halo-empty-state">
                    <Server className="halo-empty-state-icon" />
                    <h3 className="halo-empty-state-title">No degraded services</h3>
                    <p className="halo-empty-state-description">No services currently degraded.</p>
                </div>
            ) : (
